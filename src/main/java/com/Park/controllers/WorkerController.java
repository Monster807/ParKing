...
//		}
	...
	public String viewParkingBookingCalendar(Model model, @PathVariable("id") int id) {
		...
		if (bookings.size() == 0) {
			...
			return "/LW/emptyCalendar";

		} else {
			model.addAttribute("bookings", bookings);

			return "/LW/bookingCalendar";
	...
	public String showFormForAddBooking(Model theModel, @PathVariable("id") int id) {
		...
		return "/LW/bookingForm";
	}

	@PostMapping("/saveBooking/{id}")
	public String addBooking(Model model, @PathVariable("id") int id,
			@ModelAttribute("booking") @Valid BookingDto booking, BindingResult result, RedirectAttributes attr) {

		if (result.hasErrors()) {
			attr.addFlashAttribute("org.springframework.validation.BindingResult.booking", result);
		...
		if (bookingService.isOverlapping(booking, id) == false) {
			...
			emailService.sendBookingInfoForCustomer(user.getUserName(), user.getEmail(),
					booking.getCheckIn().toString().substring(0, 10), booking.getCheckOut().toString().substring(0, 10),
					parking.getParkingName(), parking.getCity(), parking.getCountrey(), parking.getAdress());

			emailService.sendBookingAlertToGuest(parking.getUser().getUserName(), parking.getUser().getEmail(),
					parking.getParkingName(), booking.getCheckIn().toString().substring(0, 10),
					booking.getCheckOut().toString().substring(0, 10));
